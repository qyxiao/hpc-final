...
N = real(ifft2(-Ksol.*chat));

Ntest = 100*99*(2*pi/L)^2*(sin(i*dx).*sin(j*dy)).^98.*(sin(j*dy).*cos(i*dx)).^2 - 100*(2*pi/L)^2*(sin(i*dx).*sin(j*dy)).^100 ...
    + 100*99*(2*pi/L)^2*(sin(i*dx).*sin(j*dy)).^98.*(cos(j*dy).*sin(i*dx)).^2 - 100*(2*pi/L)^2*(sin(i*dx).*sin(j*dy)).^100 ;
...
legend('dif L1 norm', 'dif L2 norm', 'dif L inf norm')
ylabel('Log(error)')
